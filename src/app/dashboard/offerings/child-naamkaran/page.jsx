...
export default function ChildNaamKaran() {
  ...
  return (
    <Layout isDashboard>
      ...
      >
        <h1 className="text-2xl mx-auto w-4/5 pt-10">Child Naam Karan</h1>
        ...
        <div className="flex gap-8">
          <form className="grid lg:w-2/3 lg:ml-32 mx-10 md:mx-auto pb-10">
            {Object.keys(fields).map((field) => (
              <div>
                <label>
                  {startCase(field.replaceAll("_", " "))} <span className="text-red-500">*</span>
                </label>
                <Input
                  style="w-full border-violet-600"
                  type={field === "child_date_of_birth" ? "date" : field === "child_birth_time" ? "time" : "text"}
                  name={field}
                  value={fields[field]}
                  onChange={(e) =>
                    setFields({
